...
DEPRECATED_0_7 = True # things that are deprecated in v0.7
...
def create_trie():
    ...
    print('bash toolbox/make_trie.sh "' + str(deepspeech_dir[0]) + '" "' + str(model_path) + '" ' + str('3'))
...
def clean_sentences(all_sentences, deep_create=False, analyze=False):
    ...
    for set_s in tqdm(set_sentences):
...
def multi_cleaner(inputs):
    ...
    with open(inputs[2], 'r', encoding='utf-8') as f:
        for text in tqdm(f, "thread: {0}, (lines: {1}-{2})".format(inputs[4], inputs[0], inputs[1]), unit="lines", position=inputs[4], maxinterval=1000):
