...
class DownloadWrapper(object):
    ...
    def download_callback(self, current_size, total_size, width):
        if self.t is None:
            self.t = tqdm(total=total_size, unit='B', unit_scale=True, unit_divisor=1024)
        self.t.update(width)
...
def download_files(file_url, target_path, corpus, db):
    ...
    if corpus == 'vox16' or corpus == 'vox8' or corpus == 'forscher' or corpus == 'zamia':
        ...
        start_download = not os.path.isdir(check_path)
    elif corpus == 'librivox' or corpus == "swc" or corpus == 'tatoeba' or corpus == 'tuda':
        start_download = not os.path.exists(target_file_path)
        filename = target_file_path
    if start_download:
        tools.printer(2, 'downloading', str(file_url))
        filename = wget.download(file_url, target_file_path, DownloadWrapper().download_callback)
    return filename
...
def download_single(file_url, target_path, file_name):
    os.makedirs(target_path, exist_ok=True)
    filename = wget.download(file_url, target_path + '/' + file_name, DownloadWrapper().download_callback)
...
def insert_corpora(target_path, insert=False, db=None, lang=''):
    ...
    dataset = ''
    t = tqdm(tools.scantree(target_path), "searching: " + target_path, unit="files")
    for f in t:
        for test in testing:
            if test[0] in f.path:
                dataset = test[1]
                break
        if dataset != '':
            t.set_description("found dataset: " + dataset + " at:" + f.path)
            break
    t.close()
